...
- `make init` — scaffolds a fresh `md/`, `yaml/`, `settings/page.yaml`, and
  `pdf/` in the current directory from the bundled defaults. Only needed
  when using the installed `.deb` (a git checkout already has these);
  refuses to run if any of them already exist.
...
markdown-cornell-notes init    # scaffolds md/, yaml/, settings/page.yaml, pdf/ here
...
works the same way, e.g. `markdown-cornell-notes app`.
