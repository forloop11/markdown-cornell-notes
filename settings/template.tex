...
%   |                                        |  writing)  |
%   +----------------------------------------+-----------+
%   |           Summary band, ruled (for hand writing)     |
%   +----------------------------------------------------+
%
% Usage: call \cornellpage{notes} once per page (see example at bottom).
% Header fields come from header.yaml and apply to every page; page
...
% ------------------------------------------------------------------
% Header fields sourced from header.yaml.
% Run `python3 scripts/yaml_to_header.py` to (re)generate
% build/cornell-header.tex from header.yaml before compiling; these
...
\IfFileExists{build/cornell-cue.tex}{\input{build/cornell-cue.tex}}{}
...
\IfFileExists{build/cornell-summary.tex}{\input{build/cornell-summary.tex}}{}
...
\newlength{\cnSummaryHeight} % the ruled band at the bottom of the page
...
\newlength{\cnRegionH}
\newlength{\cnCueTextH}    % height of this page's typeset cue text (0pt if none)
\newlength{\cnCueRuleTop}  % upper bound for the cue column's ruled lines
\newlength{\cnSummaryTextY}    % y-coordinate of the summary band's text area
\newlength{\cnSummaryTextHA}    % height of this page's typeset summary column-1 text (0pt if none)
\newlength{\cnSummaryTextHB}   % height of this page's typeset summary column-2 text (0pt if none)
\newlength{\cnSummaryRuleTopA}  % upper bound for the summary band's column-1 ruled lines
\newlength{\cnSummaryRuleTopB} % upper bound for the summary band's column-2 ruled lines
...
% come from \cnHdrTopic etc. (see header.yaml) and are applied to every
...
\newcommand{\cornellpage}[1]{%
  ...
  % Typeset this page's cue and summary text once, up front, so their
  % heights are known before the cue column's/summary band's ruled lines
  % are drawn -- otherwise the fixed rule-line grid runs straight through
  % the text instead of stopping above it. A page with no cue/summary
  % text gets a zero-height box, so the ruled lines fall back to spanning
  % the whole region as before.
  \setbox\cnCueBox=\vbox{\hsize=\the\cnCueTextW\linewidth=\hsize\footnotesize\noindent\cnCueText{\thepage}}%
  \setlength{\cnCueTextH}{\ht\cnCueBox}\addtolength{\cnCueTextH}{\dp\cnCueBox}%
  \setlength{\cnCueRuleTop}{\cnBodyTop}%
  \ifdim\cnCueTextH>0pt
    \addtolength{\cnCueRuleTop}{-\cnCueTextH}%
    \addtolength{\cnCueRuleTop}{-2\cnPad}%
  \fi
  ...
  % column 2, same zero-height fallback as the cue column above.
  ...
  \setlength{\cnSummaryTextHA}{\ht\cnSummaryColA}\addtolength{\cnSummaryTextHA}{\dp\cnSummaryColA}%
  \setlength{\cnSummaryRuleTopA}{\cnSummaryHeight}%
  \ifdim\cnSummaryTextHA>0pt
    \addtolength{\cnSummaryRuleTopA}{-\cnSummaryTextHA}%
    \addtolength{\cnSummaryRuleTopA}{-2\cnPad}%
  \fi

  \setlength{\cnSummaryTextHB}{0pt}%
  \setlength{\cnSummaryRuleTopB}{\cnSummaryHeight}%
  \ifvoid\cnSummaryBox
  ...
  \else
    ...
    \setlength{\cnSummaryTextHB}{\ht\cnSummaryColB}\addtolength{\cnSummaryTextHB}{\dp\cnSummaryColB}%
    \ifdim\cnSummaryTextHB>0pt
      \addtolength{\cnSummaryRuleTopB}{-\cnSummaryTextHB}%
      \addtolength{\cnSummaryRuleTopB}{-2\cnPad}%
    \fi
  \fi
  ...
  \begin{tikzpicture}
    ...
    % not a ruled boundary meant to be seen.

    % --- ruled lines: right-hand cue column, and the summary band's two
    % columns, each stopping above its own typed text for this page (if
    % any) -- see \cnCueRuleTop/\cnSummaryRuleTopA/\cnSummaryRuleTopB
    % above ---
    \cnRuledLines{\the\cnNotesWidth}{\the\cnSummaryHeight}{\the\cnPageW}{\the\cnCueRuleTop}
    \cnRuledLines{0pt}{0pt}{\the\cnHalfWidth}{\the\cnSummaryRuleTopA}
    \cnRuledLines{\the\cnHalfWidth}{0pt}{\the\cnPageW}{\the\cnSummaryRuleTopB}
...
% automatically gets the same header (from header.yaml) and the next
% page number.
% ------------------------------------------------------------------
